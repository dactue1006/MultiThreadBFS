    ...
    public void run()
    {
        ...
        try
        {
            while (!q.isEmpty())
            {
                ...
                if (node.level < Main.searchLevelLimit)
                {
                    if (node.Totalvalue == searchValue)
                    {
                        synchronized (Main.lockLevelLimitation)
                        {
                            Main.searchLevelLimit = node.level;
                            Main.lastFoundNode = node;
                            end = System.currentTimeMillis();
                            Main.lastFoundNode.searchTime = (end - start);
                            Main.lastFoundNode.winnerThreadName=Thread.currentThread().getName();
                        }
                    }
                    else
                    {
                        if (node.level + 1 < Main.searchLevelLimit)
                        {
